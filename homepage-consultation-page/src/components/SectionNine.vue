...
<template>
    <section class="flex flex-col items-center container max-w-[1160px] px-6 mx-auto py-10">
        ...
        <div class="flex flex-col md:flex-row pt-5 gap-8 w-full px-4">
            <!-- Location buttons -->
            <div class="flex flex-col flex-wrap justify-start gap-2 mb-8">
                <button v-for="loc in locations" :key="loc.name" @click="activeLocation = loc" :class="[
                    'shadow-sm whitespace-nowrap cursor-pointer text-sm font-medium transition-colors',
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-5xl">
                <div class="w-full h-50 rounded overflow-hidden shadow-sm flex-1">
                    <iframe :src="activeLocation.mapEmbed" width="100%" height="100%" style="border:0" allowfullscreen
                        loading="lazy" />
                </div>

                <div
                    class="flex flex-col justify-start text-center items-center md:items-start md:text-left min-h-[100px]">
                    <h3 class="text-2xl font-semibold text-foreground mb-3 max-w-2xl montserrat">
                        Advanced Periodontics & Implants Dentistry - {{ activeLocation.name }}
                    </h3>
                    <p class="text-base font-light text-foreground mb-2 whitespace-pre-line montserrat">
                        {{ activeLocation.address }}
                    </p>
                    <a :href="`tel:${activeLocation.phone.replace(/\\D/g, '')}`" aria-label="Call"
                        class="text-base default-blue mb-3 cursor-pointer montserrat">
                        {{ activeLocation.phone }}
                    </a>
                    <router-link to="/consultation" class="btn w-1/2 text-sm font-medium text-center">
                        Book an Appointment
                    </router-link>
                </div>
            </div>
